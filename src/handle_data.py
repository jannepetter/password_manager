...
def read_data(key):
    """
    Reads data from db and returns a list of tuples containing user data in decrypted form.
    [(id, description, password, username),]
    """
...
def change_login_password(username, password, new_username, new_password):
    ...
    try:
        ...
        for i in range(len(original_data)):
            assert original_data[i][0] == data[i][0], "id is not the same"
            assert original_data[i][1] == data[i][1], "description is not the same"
            assert original_data[i][2] == data[i][2], "password is not the same"
            assert original_data[i][3] == data[i][3], "username is not the same"
